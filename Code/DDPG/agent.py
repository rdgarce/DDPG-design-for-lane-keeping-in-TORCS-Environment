...
from DDPG.reward import *
from DDPG.conditioning import *
import tensorflow as tf
import numpy as np

class Agent:

	def __init__(self, state_space_dim = STATE_SPACE_DIM, actions_actfunc_dict = ACTIONS_ACTFUNC_DICT, actions_space_dim = ACTIONS_SPACE_DIM, clr = CLR, alr = ALR, 
											dc_fact = DC_FACT, sm_fact = SM_FACT, actor_1_dim = ACTOR_1_DIM, actor_2_dim = ACTOR_2_DIM, critic_1_dim = CRITIC_1_DIM, 
																critic_2_dim = CRITIC_2_DIM, mem_size = MEM_SIZE, batch_size = BATCH_SIZE, delay_steps = DELAY_STEPS, 
																				update_every = UPDATE_EVERY,noise = NOISE, training = False, chkpt_dir = CHKPT_DIR):
		...
		self._update_target(sm_fact = 1) #Hard Copy dei network Actor-Critic in quelli Target

	def save_agent(self, count):
	...
	def load_agent(self, count = None):
	...
	def act(self, state):
		...
		if self.training and self.noise is not None:
			...
			#bisogna fare il clip delle variabili di azione per riportarle entro i livelli massimi e minimi
			action = action.clip(ACTIONS_MIN,ACTIONS_MAX) #array definiti nel config file
	...
	def train(self):
		...
		if self.replayBuffer.mem_cntr < self.batch_size or self.training == False or self.global_steps < self.delay_steps or self.global_steps % self.update_every != 0:
			#print(f'Step {self.global_steps}: esco')
			return
		
		for _ in range(self.update_every):
			#print(f'Step {self.global_steps}, allenamento: {_}')
	...
	def store_transition(self, state, action, new_state, reward, done = False):
